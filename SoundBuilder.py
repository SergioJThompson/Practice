...
def build_sound_from_path(sound, path):
    sound.audio_seg = get_audio_seg(path)
    sound.wave_obj = to_wave_obj(sound.audio_seg)
    return sound
...
def build_sound_from_audio_seg(seg: AudioSegment):
    ...
    sound.path = ""
    sound.audio_seg = seg
    sound.wave_obj = to_wave_obj(seg)
    return sound
...
def build_missing_sound_parts(sound):
    if sound.path:
        if not sound.audio_seg:
            sound.audio_seg = get_audio_seg(sound.path)
...
def reverse(sound: Sound):
    if not sound.path and not sound.audio_seg:
        raise ValueError("SoundBuilder.reverse() was called with a Sound as argument, but the Sound contained neither\n"
                         "a path nor an AudioSegment, so the operation couldn't be performed.")
    if not sound.audio_seg:
        sound.audio_seg = get_audio_seg(sound.path)

    backwards_seg = sound.audio_seg.reverse()
    return build_sound_from_audio_seg(backwards_seg)
